...
**详细度铁律（本 skill 的核心标准）**：sr-gdd 就是"详细"。所有上游证据、分析结论、用户澄清、拍板、修订意见都必须**经过思考后落到 GDD 对应章节**，不遗漏、不概括掉、不只在对话里提一句：
...
调用时未带任何主题或材料（裸 `/sr-gdd`），**不得直接进入执行流程、不得自行扫描 workspace 找题目**。先用一段话向用户说明需要什么，并停下等输入：
...
- **界面章节**：模板 §9 必须为**每个关键界面**单独给出 ASCII 线框图（含布局、控件、UE 交互标注）+ UE 规则表（逐条编号 UE1…）+ 界面清单总览 + 流程 text 图 + 状态机（如适用）+ 全局交互规范；禁止只画一张总层级图敷衍；
