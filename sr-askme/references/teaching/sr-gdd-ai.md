...
**职责**：**首次整合**——综合 `sr-gdd-human` 成稿的规则与 `/sr-config` 的配置表整成交给程序的定稿（规则与配置字段的对应、§6 配置契约、§7 多语言文本、验收标准），不带过程留痕。**定稿通过后它不再承接修改**：成稿归档废弃，之后的改动走 `/sr-gdd-fix`（三者分工见 `../gdd-pipeline.md`）。
