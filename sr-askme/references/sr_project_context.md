...
| 状态词 | 含义 | 常用阶段 |
| --- | --- | --- |
| `unknown` | 当前材料或样本无法确定；不要用猜测填值 | `sr-analysis`、通用概念模式 |
| `待配表` | 设计需要这个数，但运行时配置尚未落成可引用的字段/记录 | `sr-concept`、`sr-analysis` 复刻规格、成稿与定稿的未落项 |
| `（参考值 X，待定）` | 成稿中需要表达数量级的可搜索占位；`X` 不是生产数值，§11 只列待定项名称 | `sr-gdd-human` |
| `表名.字段名` | 已从配置表读回的实际配置映射；只有此时才把数值当作已落配置 | `sr-gdd-ai`、`sr-gdd-fix` |

阶段转换由产物状态决定：设计阶段的 `待配表` 经过 `/sr-config` 读回后，定稿才可改写为 `表名.字段名`；没有落表就继续保留 `待配表`，不把 `unknown` 或参考占位直接当成真实值。
