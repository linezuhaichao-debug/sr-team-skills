...
2. **成稿**：`/sr-gdd-human` 基于入口产出的设计稿/复刻规格，生成**过程层工作文档**——设计结果（规则+界面）人类可读，决策过程（来源、设计核、假设/风险台账、治理引用）完整留痕，供你审核确认。
3. **落配置**：`/sr-config` 根据 GDD 把规则写成配置表（新表/加字段/改记录，带读回验收）。
4. **整合定稿**：`/sr-gdd-ai` 把 **human 成稿 + 配置表变更**整合成**没有思考决策过程的干净定稿**（功能规则、界面、配置契约、多语言文本、验收标准），交给程序实现；成稿口径不推翻，有设计问题按修订路由退回 human 定向修订。
...
**术语约定（全系列统一）**：**成稿** = `/sr-gdd-human` 的产出——**过程层工作文档（中间产物）**，设计结果人类可读 + 决策过程完整留痕，供审核确认；**定稿** = `/sr-gdd-ai` 的产出——**最终交付物**，只有设计结果与实现依据（功能意图/规则/界面/配置契约/多语言文本/验收标准），交给程序。设计稿、复刻规格是入口产出，两者都不是 GDD。设计内容的唯一撰写地是成稿；定稿发现设计问题按修订路由退回成稿定向修订。
...
| "整合定稿 / 出交给程序的干净最终版 GDD" | `/sr-gdd-ai` |
