...
2. **成稿**：`/sr-gdd-human` 基于入口产出的设计稿/复刻规格，生成**只有规则和界面**的 GDD（无证据编号、无配置契约，规则+界面线框详细可读；承担前置撰写纪律：盘点、取舍确认、映射不遗漏）。
3. **落配置**：`/sr-config` 根据 GDD 把规则写成配置表（新表/加字段/改记录，带读回验收）。
4. **整合定稿**：`/sr-gdd-ai` 把 **human 成稿 + 配置表变更**整合成最终版本 GDD（补配置契约、证据溯源、治理引用，适合正式留档；成稿口径不推翻，有问题退回 human 修订）。
...
| "写策划案 / 出 GDD / 整理成功能文档"（主线成稿步，只留规则与界面） | `/sr-gdd-human` |
| "整合定稿 / 带证据溯源的正式版 / 出完整版 GDD" | `/sr-gdd-ai` |
