...
用户问"sr 系列怎么用"、"有哪些 skill"、"/sr-concept 是什么"时，先给出下方流水线全景，再按需展开 `references/teaching/` 下对应用卡。7 张卡与各 skill 目录一一对应。
...
`/sr-config` 与 `/sr-config-heroskill` 是配置落地环节：前者把任何策划规则落成配置数据变更（新建表/字段/记录 + 读回验收），后者专精英雄技能详细设计 → 副玩法技能表。
