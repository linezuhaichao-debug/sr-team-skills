...
**主线**：sr 系列围绕一条文档生产线——创意/竞品分析产出的设计材料，最后都汇入 sr-gdd 写成功能 GDD，GDD 再派生出可读版和宣讲页：
...
2. **汇合点**：功能设计稿或复刻规格完成后，交接门选 `route_to_sr-gdd`，材料自动交给 sr-gdd——也可以不经过入口，直接 `/sr-gdd 基于 旧策划案.xlsx 出功能 GDD`。
3. **主体**：`/sr-gdd` 把材料写成实现粒度的功能 GDD（功能规则、配置契约、验收标准），关键取舍会列成清单请你逐条拍板。
4. **派生（按需）**
   - 要给开发团队看的**执行版**（无证据编号、无配置契约，只有规则和界面线框）→ `/sr-gdd-human`；
   - 要开**评审会/宣讲**的网页版（单文件 HTML，投屏可直接讲）→ `/sr-gdd-html`（输入是已定稿的 GDD）。
5. **落地（GDD 定稿后）**
   - 任何策划规则要**写成配置表**（新表/加字段/改记录）→ `/sr-config`；
   - 专精**英雄技能**配置（技能详细设计 xlsm → 副玩法技能表）→ `/sr-config-heroskill`。
