...
按任务需要加载对应 reference，不要一次性塞入所有资料。本方法目录（`game-concept-architect/`）内所有文件的内部相对路径，一律以目录根为基准解析——目录内文件互相引用时同样如此，不按文件自身目录解析：
...
按最终交付选择 template，本流程使用 `templates/idea-triage.md`：
